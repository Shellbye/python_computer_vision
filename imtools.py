...
from numpy import array, histogram, interp, dot
...
def imresize(im, sz):
    ...
    pil_im = Image.fromarray(im)  # 原书 uint8(im)
...
def histeq(im, nbr_bins=256):
    ...
    """
    # 计算图像的直方图,返回直方图均衡化后的图像，以及用来做像素值映射的累积分布函数
...
if __name__ == '__main__':
    im0 = array(Image.open('pic/empire.jpg').convert('L'))
    im20, cdf = histeq(im0)
    a = Image.fromarray(im20)
    gray()
    imshow(im20)
    show()
    # if a.mode != 'RGB':
